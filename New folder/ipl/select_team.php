...
{					$te = $_GET['t'];
			...
			$coi = mysql_fetch_array($s, MYSQL_ASSOC);
				echo $coi['coin'];
						...
						echo '<table border="1" style="position:ABSOLUTE;top:100px"><tr bgcolor="White" height="50"><td>Type</td><td>'.$te.'</td><td>Price</td></tr>';
						$i=0;$j=0;
						$pr = array();
						$ty = array();
						...
						while($team1 = mysql_fetch_array($sq1, MYSQL_ASSOC)){
							...
							echo '<tr  bgcolor="#909090"><td>'.$team1['type'].'</td>';
							echo '<td><input type="button" value="." onclick="team1('.$team1['price'].','.$i.','.$typ.')" id = "a'.$i.'"/>'.$team1['name'].'</td><td>';
						...
						if(c[i]==0){
							if((1000-sum)>=b){
										...
										sum = sum + b;
 										...
 										divNode.type = 'text/javascript';
										 divNode.innerHTML ="a"+i+".style.background = \"url(checked.png)\"";
							...
							c[i]=0;
							sum = sum - b;
             						var divNode2 = document.createElement("script");
 										divNode2.type = 'text/javascript';
										 divNode2.innerHTML ="a"+i+".style.background = \"\";";
